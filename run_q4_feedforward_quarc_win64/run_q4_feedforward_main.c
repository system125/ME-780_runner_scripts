...
typedef RT_MODEL_run_q4_feedforward_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * run_q4_feedforward(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(run_q4_feedforward_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: run_q4_feedforward -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
