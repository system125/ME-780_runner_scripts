...
typedef RT_MODEL_bodeTestRunner_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * bodeTestRunner(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(bodeTestRunner_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://bodeTestRunner:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: bodeTestRunner -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
